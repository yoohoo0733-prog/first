...
export default function FixedItemManager() {
  ...
  const getAccountName = (id: string) => accounts.find(a => a.id === id)?.name ?? '알 수 없음';

  const getAdj = (item: FixedItem) => {
  ...
  const startNew = (section: 'salary' | 'card' | 'regular') => {
    setError(null);
    setEditState({
      id: 'new',
      section,
  ...
  // Inline form — used both for adding new items and editing existing ones
  const InlineForm = ({ label }: { label: string }) => {
    ...
    // Preview adjustment for the currently selected day
    const previewAdj = useMemo(() => {
      if (editState.day === 'last') {
        const lastDay = getDaysInMonth(today.year, today.month);
        return getItemAdjustedDay(section, today.year, today.month, lastDay);
      }
      const d = parseInt(editState.day, 10);
      if (isNaN(d) || d < 1 || d > 31) return null;
      return getItemAdjustedDay(section, today.year, today.month, d);
    }, [editState.day, section]);
    ...
    return (
      <div className="border border-sky-200 bg-sky-50/40 rounded-xl p-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <input
              ...
              placeholder={section === 'salary' ? '예: 홍길동 월급' : section === 'card' ? '예: 신한카드' : '항목명'}
          ...
          <div>
            ...
            <input
              ...
              onChange={e => setEditState(s => s ? { ...s, amount: e.target.value.replace(/[^0-9]/g, '') } : null)}
            />
          </div>
          <div>
            ...
            <select
              ...
              value={editState.day}
              onChange={e => setEditState(s => s ? { ...s, day: e.target.value } : null)}
            >
            ...
            {previewAdj && previewAdj.wasAdjusted && (
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setEditState(s => s ? { ...s, accountId: e.target.value } : null)}
            >
        ...
        <div className="flex gap-2">
          <button onClick={handleSave} className="btn-primary flex items-center gap-1.5 text-sm py-2">
            <Check className="w-3.5 h-3.5" />
            {editState.id === 'new' ? '추가' : '저장'}
          </button>
          <button onClick={cancelEdit} className="btn-secondary text-sm py-2">취소</button>
        </div>
      </div>
    );
  };

  // Single item row in view mode
  const ItemRow = ({ item, deletable }: { item: FixedItem; deletable: boolean }) => {
    const adj = getAdj(item);
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {salaryItems.map(item =>
          isEditingItem(item.id)
            ? <InlineForm key={item.id} label="월급 수정" />
            : <ItemRow key={item.id} item={item} deletable={false} />
        )}

        {isAddingTo('salary') && <InlineForm label="월급 추가" />}
      </div>
      ...
      <div className="space-y-2">
        ...
        {cardItems.map(item =>
          isEditingItem(item.id)
            ? <InlineForm key={item.id} label="카드대금 수정" />
            : <ItemRow key={item.id} item={item} deletable={true} />
        )}

        {isAddingTo('card') && <InlineForm label="카드 추가" />}
      </div>
      ...
      <div className="space-y-2">
        ...
        {regularItems.map(item =>
          isEditingItem(item.id)
            ? <InlineForm key={item.id} label="항목 수정" />
            : <ItemRow key={item.id} item={item} deletable={true} />
        )}

        {isAddingTo('regular') && <InlineForm label="항목 추가" />}
      </div>
    </div>
  );
}
